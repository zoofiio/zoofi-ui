...
import { NATIVE_TOKEN_ADDRESS, PROTOCOL_SETTINGS_ADDRESS, USB_ADDRESS, USBSymbol, VAULT_QUERY_ADDRESS, VaultConfig } from '@/config/swap'
...
export function StableLVaultAdvance({ vc }: { vc: VaultConfig }) {
  ...
  const assetPrice = vs.latestPrice
  const xPrice = prices[vc.xTokenAddress]
  const balances = useBalances()
  const balance = balances[vc.assetTokenAddress] || 0n
  const usbBalance = balances[USB_ADDRESS[chainId]] || 0n
  const xBalance = balances[vc.xTokenAddress] || 0n
  // redeem state
  ...
  const showXInput = isRedeemByUSB ? '0' : redeemFocus == vc.xTokenSymbol ? xAmount : formatEther(redeemPairXtokenInVaule)
  ...
  const redeemOut = isRedeemByXtoken ? redeemOutByXtokenVaule : isRedeemByUSB ? redeemOutByUsbVaule : getBigint(redeemPairOutData, [1])

  const redeemPrepareConfig = useMemo(() => {
    const base: Parameters<typeof ApproveAndTx>[0]['config'] = {
      abi: abiStableVault,
      address: vc.vault,
      functionName: redeemActive == USBSymbol ? 'redeemByPairsWithExpectedUsbAmount' : 'redeemByPairsWithExpectedMarginTokenAmount',
  ...
  return (
    <div className={twMerge('card relative h-[480px]')}>
        ...
        <Tip
          node={
            <div className={twMerge('text-sm leading-5 flex items-center gap-1.5 rounded-full w-fit')}>
              ...
              <div className={twMerge(!isAdjustment ? 'text-green-500' : 'text-red-500')}>{isAdjustment ? 'Adjustment Model' : 'Stability Model'}</div>
            </div>
          }
        >
          {!isAdjustment ? '' : tab == 'Mint' ? `Minting ${USBSymbol} alone is not feasible` : 'Redeeming USDBx alone is not feasible'}
        </Tip>
      </div>
            ...
            },
            {
              ...
              content: (
                <div className='mt-4'>
                  ...
                  <AssetInput asset={vc.assetTokenSymbol} amount={formatEther(redeemOut)} exchange={displayBalance((assetPrice * redeemOut) / DECIMAL)} readonly />
                  <ApproveAndTx
                    tx='Redeem'
                    className='mt-6 w-full'
                    ...
                    disabled={
                      ...
                      (!isRedeemByUSB && !isRedeemByXtoken && (parseEthers(showUsbInput) == 0n || parseEthers(showXInput) == 0n)) ||
